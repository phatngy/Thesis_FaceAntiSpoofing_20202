...
def random_cropping(image, target_shape=(32, 32, 3), roi=None):
...
def color_augumentor(image, target_shape=(32, 32, 3), is_infer=False, roi=None):
    if is_infer:
        ...
        image = random_cropping(image, target_shape, roi=roi)
        return image

def depth_augumentor(image, target_shape=(32, 32, 3), is_infer=False, roi=None):
    if is_infer:
        augment_img = iaa.Sequential([
            iaa.Fliplr(0),
        ])
...
def ir_augumentor(image, target_shape=(32, 32, 3), is_infer=False, roi=None):
    if is_infer:
        augment_img = iaa.Sequential([
            iaa.Fliplr(0),
        ])
