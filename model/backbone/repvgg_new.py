...
def create_RepVGG_A0(deploy=False, num_blocks=[2, 4, 14, 1]):
    return RepVGG(num_blocks=num_blocks, num_classes=1000,
...
def repvgg_model_convert(model:torch.nn.Module, save_path=None, do_copy=True):
    dummy = torch.randn((128, 9, 48, 48))

    if do_copy:
        model_infer = copy.deepcopy(model)
    for module in model_infer.modules():
        if hasattr(module, 'switch_to_deploy'):
            module.switch_to_deploy()
    if save_path is not None:
        torch.save(model_infer.state_dict(), save_path)
    
    # model.eval()
    # model_infer.eval()

    # out1, _, _ = model.forward(dummy)
    # out2, _, _ = model_infer.forward(dummy)
    # print('diff: ', (out2-out1).abs().sum() / (128), out1.shape , out2.shape) 
    del model
    return model_infer